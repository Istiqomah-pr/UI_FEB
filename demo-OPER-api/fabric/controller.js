...
const connectToNetwork = async () => {
  ...
  const walletPath = path.join(__dirname, "Org2");
...
try {
  ...
  console.log(`Wallet path: ${walletPath}`);
    ...
    const userIdentity = await wallet.get("oper");
    if (userIdentity) {
      console.log(
        'An identity for the user "oper" already exists in the wallet'
      );
      return res.status(401).send("duplicate");
    }
    ...
    await wallet.put("oper", x509Identity);
    console.log(
      'Successfully registered and enrolled user "oper" and imported it into the wallet'
    );
    return res
      .status(200)
      .send(
    ...
    console.error(`Failed to register user "oper": ${error}`);
    next(error);
  }
};
...
exports.createAircraft = async (req, res, next) => {
    try {
      ...
      JSON.parse((await getMetadata.evaluate()).toString());
    } catch (error) {
